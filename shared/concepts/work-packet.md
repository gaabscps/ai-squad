...
The **Work Packet** is the only structured channel from the orchestrator to a Subagent. It is a JSON file with a fixed schema, written by the orchestrator to `.agent-session/<spec_id>/inputs/<dispatch_id>.json`, and passed to the Subagent via the convention `WorkPacket: <path>` in the prompt of the `Agent` tool.

It is the **yang to the Output Packet's yin**: same FS layout (`.agent-session/<spec_id>/{inputs,outputs}/`), same `dispatch_id` shared between paired packets, same validation-gate discipline, same minimal-handoff principle.
...
| `spec_ref` | string | yes | Path to the Spec file: `.agent-session/<spec_id>/spec.md`. |
...
| FS path | `.agent-session/<spec_id>/inputs/<dispatch_id>.json` | `.agent-session/<spec_id>/outputs/<dispatch_id>.json` |
