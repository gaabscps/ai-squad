...
def test_seal_is_idempotent(tmp_path):
    ...
    (proj / "chatX.jsonl").write_text(json.dumps({
        ...
        "message": {"id": "m1", "model": "claude-opus-4-8",
                    "usage": {"input_tokens": 10, "output_tokens": 5}}}) + "\n")
    env = {**os.environ, "HOME": str(tmp_path / "home")}
    args = [sys.executable, str(_SEAL), "OBS-009", "chatX", str(tmp_path / ".agent-session")]
    code1 = subprocess.run(args, env=env, capture_output=True, text=True).returncode
    assert code1 == 0
    code2 = subprocess.run(args, env=env, capture_output=True, text=True).returncode
    assert code2 == 0
    yml_text = (session_dir / "session.yml").read_text()
    assert yml_text.count("chatX") == 1
