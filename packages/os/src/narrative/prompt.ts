...
export function buildNarrativePrompt(source: NarrativeSource, outputLocale: string | null): string {
  ...
  return [
    ...
    "Cubra TODAS as frentes/steps trabalhados na sessão; não omita partes só por concisão — prefira mais itens em 'changes' a um resumo genérico.",
    ...
    "",
    `## Conversa da sessão (usuário + assistente) — use para o 'porquê' e para cobrir todas as frentes\n${source.reasoning || "(não disponível)"}`,
    "",
